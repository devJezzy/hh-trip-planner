...
const MyComponent: React.FC = () => {
  ...
  const tripTitle = travelDays == "1" ? `${travelStyle} in` : `${travelDays} Days ${travelStyle} Trip in`;
  ...
  useEffect(() => {
    if (travelDays && destination && travelStyle) {
      fetchItinerary(`${travelStyle} trip to ${destination} for ${travelDays} days`);
    }
  }, [travelDays, destination, travelStyle, fetchItinerary]);

  const transformItinerary = useCallback(async (originalData: DayItinerary[]) => {
    for (const dayData of originalData) {
      ...
      for (const item of dayData.Itinerary) {
        ...
        try {
          ...
          transformedItems.push({ time: Time, title: Activity, description: Comments, imageSrc, iconSrc, name: Name });
        } catch (error) {
          console.error(`Error fetching image for activity ${Activity}:`, error);
          transformedItems.push({ time: Time, title: Activity, description: Comments, imageSrc: "", iconSrc, name: Name });
        }
      }
      setItineraryData(prevData => [...prevData, { day: dayData.Day, items: transformedItems }]);
    }
    setIsLoaded(true);
  }, []);
            ...
            <header className="flex flex-col self-center pb-5 max-sm:w-[90%]">
              <div className="trip-planner-sec px-4 pt-12 max-md:max-w-full">
                <div className="name-logo-sec flex flex-row ">
                  ...
                  <Image className="mx-8 max-md:mx-2 max-md:w-25" src="/travel-bot.png" alt="Main" width={150} height={50} />
                </div>
                      ...
                      <div className="flex flex-wrap gap-2 items-start pr-20 pb-4 max-md:pr-5">
                        <div className="flex flex-col justify-center px-2 rounded-full bg-neutral-100 max-w-[300px]">
                          <div className="flex flex-col justify-center">
                            <div className="justify-center">{travelDays == "1" ? "day" : "days"}</div>
                          </div>
                        </div>
                            ...
                            <div className="flex flex-col max-md:max-w-full" key={index}>
                              ...
                              <div className="flex flex-col mt-6 max-md:max-w-full">
                                {day.items.map((item, idx) => (
                                  <ItineraryItem key={idx} time={item.time} title={item.title} description={item.description} iconSrc={item.iconSrc} imageSrc={item.imageSrc} name={item.name} destination={city} />
                                ))}
